...
export interface StudentRow { id: number; student_id: string; last_name: string; first_name: string; grade: number; section: string | null; house_team: string | null; counselor: string | null; gpa: number; total_points: number; conduct_status: string; observations: string; date_of_birth: string | null; parent_name: string | null; parent_phone: string | null; gender: string | null; created_at: Date; }
...
export async function initializeDatabase() {
  ...
  const tableQueries = [
    { name: 'users', sql: `CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, username TEXT UNIQUE NOT NULL, password TEXT NOT NULL, role TEXT NOT NULL DEFAULT 'user', first_name TEXT, last_name TEXT, email TEXT, classroom TEXT, phone TEXT, profile_picture TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)` },
    { name: 'students', sql: `CREATE TABLE IF NOT EXISTS students (id SERIAL PRIMARY KEY, student_id TEXT UNIQUE NOT NULL, last_name TEXT NOT NULL, first_name TEXT NOT NULL, grade INTEGER DEFAULT 9, section TEXT, house_team TEXT, counselor TEXT, gpa REAL DEFAULT 0.0, total_points INTEGER DEFAULT 100, conduct_status TEXT DEFAULT 'Good', observations TEXT DEFAULT '', date_of_birth TEXT, parent_name TEXT, parent_phone TEXT, gender TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)` },
