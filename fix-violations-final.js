...
async function fixViolations() {
  ...
  // Create new clean table with UNIQUE constraint
  db.run('DROP TABLE IF EXISTS violations_clean');
  db.run(`
    CREATE TABLE violations_clean (
  ...
  const stmt = db.prepare('INSERT INTO violations_clean (category, violation_type, description, points_deduction, default_consequence, min_oss_days, max_oss_days) VALUES (?, ?, ?, ?, ?, ?, ?)');
