...
async function updateViolations() {
  ...
  const result = db.exec('SELECT COUNT(*) as count FROM violations');
  console.log('Total violations after update:', result[0].values[0][0]);

  db.close();
  console.log('Violations updated successfully!');
}

updateViolations().catch(console.error);